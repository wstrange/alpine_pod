...
void main() {
  ...
  withServerpod('Notification Endpoint Tests', (sessionBuilder, endpoints) {
    ...
    test('Register, update, and unregister FCM tokens', () async {
      ...
      final session = sessionBuilder.build();
      var stored = await FcmToken.db.findFirstRow(
        session,
        where: (t) => t.token.equals(token1),
      );
      ...
      var oldStored = await FcmToken.db.findFirstRow(
        session,
        where: (t) => t.token.equals(token1),
      );
      expect(oldStored, isNull);

      var newStored = await FcmToken.db.findFirstRow(
        session,
        where: (t) => t.token.equals(token2),
      );
      ...
      var deletedStored = await FcmToken.db.findFirstRow(
        session,
        where: (t) => t.token.equals(token2),
      );
    ...
    test('Subscribe, list, and unsubscribe event types', () async {
      ...
      // 2. Subscribe to Hike
      await endpoints.notification.subscribeToEventType(
        userAuth,
        'Hike',
        subscribe: true,
      );
