...
class UserProfileEndpoint extends UserProfileEditBaseEndpoint {
  @override
  Future<UserProfileModel> setUserImage(final Session session, final ByteData image) async {
    ...
    if (profile == null) {
      ...
      final member = await Member.db.findFirstRow(session, where: (t) => t.id.equals(authUserId));
      ...
      if (email == null) {
        ...
        final emailAccount = await EmailAccount.db.findFirstRow(session, where: (t) => t.authUserId.equals(authUserId));
      ...
      await AuthServices.instance.userProfiles.createUserProfile(
        session,
        authUserId,
        UserProfileData(email: email ?? 'user@example.com', userName: userName ?? 'User'),
      );
    }
